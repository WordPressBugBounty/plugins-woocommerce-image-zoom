...
if ($attachment_ids && $product->get_image_id()) {
	if ('on' === $add_thumb_to_gallery && has_post_thumbnail(get_the_id())) {
	...
	foreach ($attachment_ids as $key => $attachment_id) {
		...
		$image             = wp_get_attachment_image(
			...
			apply_filters(
				...
				array(
					...
					'class'                   => esc_attr($attachment_id === get_post_thumbnail_id() ? 'wpb-wiz-featured-image' : ''),
				),
				$attachment_id,
				$image_size,
				$main_image
		...
		echo '<a class="woocommerce-product-gallery__image" href="#" data-image="' . esc_url($thumbnail_src[0]) . '" data-large_image="' . esc_url($full_src[0]) . '">' . wp_kses_post($image) . '</a>';
	}
	echo '</div>';
}
